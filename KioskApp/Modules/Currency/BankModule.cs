...
using KioskApp.Modules.Administration.Extensions;
using KioskApp.Modules.Currency.Extensions;

#endregion USING_DIRECTIVES

namespace KioskApp.Modules.Currency
{
    ...
    [UsageExamples("!bank",
                   "!bank @Someone")]
    public class BankModule : AppModule
    {
                                         ...
                                         [Description("User.")] DiscordUser user = null)
        {
            ...
            {
                BankAccounts account = await db.BankAccounts.FindAsync((long)ctx.Guild.Id, (long)user.Id);
        ...
        #endregion COMMAND_BANK_BALANCE
        ...
        #endregion COMMAND_BANK_CURRENCY
        ...
        public async Task RegisterAsync(CommandContext ctx)
        {
            using (DatabaseContext db = this.Database.CreateContext())
            {
                if (await db.BankAccounts.FindAsync((long)ctx.Guild.Id, (long)ctx.User.Id) is null)
                    db.BankAccounts.Add(new BankAccounts() { GuildId = ctx.Guild.Id, UserId = ctx.User.Id });
                else
            ...
            await this.InformAsync(ctx, StaticDiscordEmoji.MoneyBag, $"Account opened for you, {ctx.User.Mention}! Since WM bank is so generous, you get {BankAccounts.StartingBal} {this.Shared.GetGuildConfig(ctx.Guild.Id).Currency ?? "credits"} for free.");
        }

        #endregion COMMAND_BANK_REGISTER
        ...
        public async Task GetLeaderboardAsync(CommandContext ctx)
        {
            ...
            foreach (BankAccounts account in topAccounts)
            {
                try
                {
        ...
        #endregion COMMAND_BANK_TOPGLOBAL
        ...
        #endregion COMMAND_BANK_TRANSFER
                                         ...
                                         [Description("Globally delete?")] bool global = false)
        {
            using (DatabaseContext db = this.Database.CreateContext())
            {
                ...
                {
                    BankAccounts acc = db.BankAccounts.SingleOrDefault(a => a.GuildId == ctx.Guild.Id && a.UserId == user.Id);
                    if (acc is null)
        ...
        #endregion COMMAND_BANK_UNREGISTER
    }
}